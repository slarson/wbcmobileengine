...
ofxHttpResponse ofxHttpUtils::doPostFormWithResponse(ofxHttpForm & form){
...
void ofxHttpUtils::getUrlWithPort(string url, uint16_t port){
	try{
		URI uri(url.c_str());
		std::string path(uri.getPathAndQuery());
		if (path.empty()) path = "/";
		//printf("[OFXHTTPUTIL] port: %d, used: %d\n", uri.getPort(), port);
...
ofxHttpResponse ofxHttpUtils::getUrlWithResponse(string url){
	try{
		URI uri(url.c_str());
		std::string path(uri.getPathAndQuery());
		if (path.empty()) path = "/";
