...
void CurveHop :: draw ()
{
	...
	GLfloat *curveTrianglesNormal	= new GLfloat[ n * 6 ];
	GLfloat *curveTrianglesColor	= new GLfloat[ n * 8 ];
//	GLfloat *curve
	...
	for( int j=j1; j<=j2; j++ )									// draw bezier connections.
	{
		...
		if( j == j1 )
		{
			...
			curveTrianglesNormal[ k * 6 + 0 ] = 0;
			curveTrianglesNormal[ k * 6 + 1 ] = 0;
			curveTrianglesNormal[ k * 6 + 2 ] = 0;
			curveTrianglesNormal[ k * 6 + 3 ] = 0;
			curveTrianglesNormal[ k * 6 + 4 ] = 0;
			curveTrianglesNormal[ k * 6 + 5 ] = 0;
			
			curveTrianglesColor[ k * 8 + 0 ] = 1.0;
			curveTrianglesColor[ k * 8 + 1 ] = 1.0;
			curveTrianglesColor[ k * 8 + 2 ] = 1.0;
			curveTrianglesColor[ k * 8 + 3 ] = 0.6;
			curveTrianglesColor[ k * 8 + 4 ] = 1.0;
			curveTrianglesColor[ k * 8 + 5 ] = 1.0;
			curveTrianglesColor[ k * 8 + 6 ] = 1.0;
			curveTrianglesColor[ k * 8 + 7 ] = 0.6;
		}
		else
		{
			...
			curveTrianglesNormal[ k * 6 + 0 ] = look.x;
			curveTrianglesNormal[ k * 6 + 1 ] = look.y;
			curveTrianglesNormal[ k * 6 + 2 ] = look.z;
			curveTrianglesNormal[ k * 6 + 3 ] = look.x;
			curveTrianglesNormal[ k * 6 + 4 ] = look.y;
			curveTrianglesNormal[ k * 6 + 5 ] = look.z;
	...
	drawCurveTriangleStrip( curveTriangles, curveTrianglesNormal, curveTrianglesColor, n * 2 );
	
	delete[] curveLineStrip;
	delete[] curveColor;
	delete[] curveTriangles;
	delete[] curveTrianglesColor;
}
...
void CurveHop :: drawCurveTriangleStrip( GLfloat *vertex, GLfloat *normal, GLfloat *color, int length )
{
